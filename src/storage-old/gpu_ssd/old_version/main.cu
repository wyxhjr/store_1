...
static double launchMoveKernelLoop(void *fileMap, BufferPtr destination, size_t pageSize,
                                   const Settings &settings) {
...
static double launchNvmKernel(const Controller &ctrl, BufferPtr destination,
                              const Settings &settings, const cudaDeviceProp &prop) {
  QueuePair queuePair;
  ...
  auto err = cudaMemcpy(deviceQueue.get(), &queuePair, sizeof(QueuePair),
  ...
  try {
    ...
    if (settings.doubleBuffered) {
      readDoubleBuffered<<<1, settings.numThreads>>>(
          ...
          destination.get(), totalChunks, settings.startBlock, ec, times.get());
    } else {
...
int main(int argc, char **argv) {
  ...
  if (settings.cudaDeviceId != 0) {
    ...
    SCIRegisterPCIeRequester(sd, settings.adapter, settings.bus, settings.devfn,
                             SCI_FLAG_PCIE_REQUESTER_GLOBAL, &err);
    if (err != SCI_ERR_OK) {
  ...
  try {
#ifdef __DIS_CLUSTER__
    Controller ctrl(settings.controllerId, settings.nvmNamespace, settings.adapter,
                    settings.segmentId++);
#else
    ...
    if (err != cudaSuccess) {
      throw error(
          string("Unexpected error while mapping IO memory (cudaHostRegister): ") +
          cudaGetErrorString(err));
    }
    ...
    SCIUnregisterPCIeRequester(sd, settings.adapter, settings.bus, settings.devfn, 0,
                               &err);
    SCIClose(sd, 0, &err);
    SCITerminate();
#endif
