...
struct tlb_entry {
  ...
  __forceinline__ __host__ __device__ tlb_entry() {
    init();
  }

  __forceinline__ __host__ __device__ void init() {
    global_id = 0;
    state.store(0, simt::memory_order_relaxed);
    page = nullptr;
  }

  __forceinline__ __device__ void release(const uint32_t count) {
    //		    if (global_id == 515920192)
    //			printf("--(2)st: %llx\tcount: %llu\n", (unsigned long long)
    // state.load(simt::memory_order_relaxed), (unsigned long long) count);

    state.fetch_sub(count, simt::memory_order_release);
  }

  __forceinline__ __device__ void release() {
    if (page != nullptr) {
      //		    if (global_id == 515920192)
      //			printf("--(1)st: %llx\tcount: %llu\n", (unsigned long
      // long) state.load(simt::memory_order_relaxed), (unsigned long long) 1);
...
struct tlb {
  ...
  __forceinline__ __host__ __device__ tlb() {}
  /*
    __forceinline__
    __device__
    tlb(array_d_t<T>* a) { init(a); }
  */
  __forceinline__ __device__ void init(array_d_t<T> *a) {
    // SYNC;
    //	__syncthreads();
    if (n) {
      size_t tid = TID;
      if (tid == 0) array = a;
      for (; tid < n; tid += BLKSIZE) entries[tid].init();
    }
  ...
  __forceinline__ __device__ void fini() {
    //      SYNC;
    //	__syncthreads();

    if (n) {
      size_t tid = TID;
      for (; tid < n; tid += BLKSIZE) entries[tid].release();
    }
    //	__syncthreads();
  ...
  __forceinline__ __device__ ~tlb() {}

  __forceinline__ __device__ T *acquire(const size_t i, const size_t gid, size_t &start,
                                        size_t &end, range_d_t<T> *range,
                                        const size_t page_) {
    ...
    if (lane == master) {
      ...
      do {
        ...
        if ((entry->page != nullptr) && (gid == entry->global_id)) {
          ...
          //(unsigned long long) st, (unsigned long long) (st & VALID_), (unsigned long
          // long) count);
          ...
          //		    if (gid == 515920192)
          //			printf("++(2)st: %llx\tst&Val: %llx\tVal: %llx\tcount:
          //%llu\n", (unsigned long long) st, (unsigned long long) (st & VALID_),
          //(unsigned long long) VALID_, (unsigned long long) count);
          //
          ...
          if (++c % 100000 == 0)
            printf(
                "c: %llu\ttid: %llu\twanted_gid: %llu\tgot_gid: %llu\tst: %llx\tst&0x7: "
                "%llx\n",
                (unsigned long long)c, (unsigned long long)(TID), (unsigned long long)gid,
                (unsigned long long)entry->global_id, (unsigned long long)st,
                (unsigned long long)(st & 0x7fffffff));
  ...
  __forceinline__ __device__ void release(const size_t gid) {
    // size_t gid = array->get_page_gid(i);
    uint32_t lane = lane_id();
    uint32_t mask = __activemask();
    uint32_t eq_mask = __match_any_sync(mask, gid);
    ...
    size_t ent = gid % n;
    tlb_entry<_scope> *entry = entries + ent;
    if (lane == master) entry->release(count);
    __syncwarp(eq_mask);
  }
};
...
struct bam_ptr_tlb {
  ...
  __forceinline__ __host__ __device__ bam_ptr_tlb(array_d_t<T> *a,
                                                  tlb<T, n, _scope, loc> *t) {
    init(a, t);
  }

  __forceinline__ __device__ ~bam_ptr_tlb() {
    fini();
  }

  __forceinline__ __host__ __device__ void init(array_d_t<T> *a,
                                                tlb<T, n, _scope, loc> *t) {
    array = a;
    tlb_ = t;
  }

  __forceinline__ __device__ void fini(void) {
    if (addr) {
      tlb_->release(gid);
      addr = nullptr;
    }
  ...
  __forceinline__ __device__ void update_page(const size_t i) {
    ////printf("++++acquire: i: %llu\tpage: %llu\tstart: %llu\tend: %llu\trange: %llu\n",
    //            (unsigned long long) i, (unsigned long long) page, (unsigned long long)
    //            start, (unsigned long long) end, (unsigned long long) range_id);
    fini();  // destructor
    range_d_t<T> *range;
    size_t page;
    array->get_page_gid(i, range, page, gid);
    addr = (T *)tlb_->acquire(i, gid, start, end, range, page);
    //        //printf("----acquire: i: %llu\tpage: %llu\tstart: %llu\tend: %llu\trange:
    //        %llu\n",
    //            (unsigned long long) i, (unsigned long long) page, (unsigned long long)
    //            start, (unsigned long long) end, (unsigned long long) range_id);
  }

  __forceinline__ __device__ T operator[](const size_t i) const {
    if ((i < start) || (i >= end)) {
      update_page(i);
    }
    return addr[i - start];
  ...
  __forceinline__ __device__ T &operator[](const size_t i) {
    if ((i < start) || (i >= end)) {
      update_page(i);
    }
    return addr[i - start];
...
struct bam_ptr {
  ...
  __forceinline__ __host__ __device__ bam_ptr(array_d_t<T> *a) {
    init(a);
  }

  __forceinline__ __host__ __device__ ~bam_ptr() {
    fini();
  }
  ...
  __forceinline__ __host__ __device__ void fini(void) {
    if (page) {
  ...
  __forceinline__ __host__ __device__ T *update_page(const size_t i) {
    ////printf("++++acquire: i: %llu\tpage: %llu\tstart: %llu\tend: %llu\trange: %llu\n",
    //            (unsigned long long) i, (unsigned long long) page, (unsigned long long)
    //            start, (unsigned long long) end, (unsigned long long) range_id);
    fini();  // destructor
  ...
  __forceinline__ __host__ __device__ T operator[](const size_t i) const {
    if ((i < start) || (i >= end)) {
  ...
  __forceinline__ __host__ __device__ T &operator[](const size_t i) {
    if ((i < start) || (i >= end)) {
      update_page(i);
    }
    return addr[i - start];
...
struct page_cache_d_t {
  ...
  // uint64_t* prp_list;              //len = num of pages in cache if page_size >
  // ctrl.page_size *2
  ...
  __forceinline__ __device__ cache_page_t *get_cache_page(const uint32_t page) const;

  __forceinline__ __device__ uint32_t find_slot(uint64_t address, uint64_t range_id,
                                                const uint32_t queue_);
};

__device__ void read_data(page_cache_d_t *pc, QueuePair *qp, const uint64_t starting_lba,
                          const uint64_t n_blocks, const unsigned long long pc_entry);
__device__ void write_data(page_cache_d_t *pc, QueuePair *qp, const uint64_t starting_lba,
                           const uint64_t n_blocks, const unsigned long long pc_entry);

__forceinline__ __device__ uint64_t get_backing_page_(const uint64_t page_start,
                                                      const size_t page_offset,
                                                      const uint64_t n_ctrls,
...
__forceinline__ __device__ uint64_t get_backing_ctrl_(const size_t page_offset,
                                                      const uint64_t n_ctrls,
                                                      const data_dist_t dist) {
  uint64_t ctrl;
...
__global__ void __flush(page_cache_d_t *pc) {
  ...
  if (page < pc->n_pages) {
    ...
    uint32_t expected_state = pc->ranges[previous_range][previous_address].state.load(
        simt::memory_order_relaxed);

    uint32_t d = expected_state & DIRTY;
    uint32_t smid = get_smid();
    if (d) {
      uint64_t ctrl = get_backing_ctrl_(previous_address, pc->n_ctrls,
                                        pc->ranges_dists[previous_range]);
      // uint64_t get_backing_page(const uint64_t page_start, const size_t page_offset,
      // const uint64_t n_ctrls, const data_dist_t dist) {
      uint64_t index =
          get_backing_page_(pc->ranges_page_starts[previous_range], previous_address,
                            pc->n_ctrls, pc->ranges_dists[previous_range]);
      ...
      if (ctrl == ALL_CTRLS) {
        for (ctrl = 0; ctrl < pc->n_ctrls; ctrl++) {
          ...
          write_data(pc, (c->d_qps) + queue, (index * pc->n_blocks_per_page),
                     pc->n_blocks_per_page, page);
        }
      } else {
        Controller *c = pc->d_ctrls[ctrl];
        uint32_t queue = smid % (c->n_qps);
...
struct page_cache_t {
  ...
  void print_reset_stats(void) {
    uint64_t v = 0;
    cuda_err_chk(cudaMemcpy(&v, pdt.extra_reads,
                            sizeof(simt::atomic<uint64_t, simt::thread_scope_device>),
                            cudaMemcpyDeviceToHost));

    cuda_err_chk(cudaMemset(pdt.extra_reads, 0,
                            sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));
  ...
  void add_range(range_t<T> *range) {
    ...
    cuda_err_chk(cudaMemcpy(pdt.ranges_page_starts, h_ranges_page_starts,
                            pdt.n_ranges * sizeof(uint64_t), cudaMemcpyHostToDevice));
    cuda_err_chk(cudaMemcpy(pdt.ranges, h_ranges, pdt.n_ranges * sizeof(pages_t),
                            cudaMemcpyHostToDevice));
    cuda_err_chk(cudaMemcpy(pdt.ranges_dists, h_ranges_dists,
                            pdt.n_ranges * sizeof(data_dist_t), cudaMemcpyHostToDevice));
    cuda_err_chk(
  ...
  page_cache_t(const uint64_t ps, const uint64_t np, const uint32_t cudaDevice,
    ...
    ctrl_counter_buf = createBuffer(
        sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
    q_head_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>),
                              cudaDevice);
    q_tail_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>),
                              cudaDevice);
    q_lock_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>),
                              cudaDevice);
    extra_reads_buf = createBuffer(
        sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
    pdt.ctrl_counter =
        (simt::atomic<uint64_t, simt::thread_scope_device> *)ctrl_counter_buf.get();
    pdt.page_size = ps;
    ...
    pdt.extra_reads =
        (simt::atomic<uint64_t, simt::thread_scope_device> *)extra_reads_buf.get();
    ...
    for (size_t k = 0; k < pdt.n_ctrls; k++)
      cuda_err_chk(cudaMemcpy(pdt.d_ctrls + k, &(ctrls[k]->d_ctrl_ptr),
    ...
    //     cuda_err_chk(cudaMemcpy(d_ctrls+k, &(ctrls[k]->d_ctrl_ptr),
    //     sizeof(Controller*), cudaMemcpyHostToDevice));
    ...
    for (size_t i = 0; i < np; i++) tps[i].page_take_lock = FREE;
    cuda_err_chk(cudaMemcpy(pdt.cache_pages, tps, np * sizeof(cache_page_t),
                            cudaMemcpyHostToDevice));
    delete tps;
    ...
    this->pages_dma =
        createDma(ctrl.ctrl, NVM_PAGE_ALIGN(cache_size, 1UL << 16), cudaDevice);
    pdt.base_addr = (uint8_t *)this->pages_dma.get()->vaddr;
    std::cout << "pages_dma: " << std::hex << this->pages_dma.get()->vaddr << "\t"
              << this->pages_dma.get()->ioaddrs[0] << std::endl;
    std::cout << "HEREN\n";
    const uint32_t uints_per_page = ctrl.ctrl->page_size / sizeof(uint64_t);
    if ((pdt.page_size > (ctrl.ctrl->page_size * uints_per_page)) || (np == 0) ||
        (pdt.page_size < ctrl.ns.lba_data_size))
    ...
    if (ps <= this->pages_dma.get()->page_size) {
      ...
      std::cout << np << " " << sizeof(uint64_t) << " " << how_many_in_one << " "
                << this->pages_dma.get()->n_ioaddrs << std::endl;
      ...
      if (temp == NULL) std::cout << "NULL\n";

      for (size_t i = 0; (i < this->pages_dma.get()->n_ioaddrs); i++) {
        for (size_t j = 0; (j < how_many_in_one); j++) {
          temp[i * how_many_in_one + j] =
              ((uint64_t)this->pages_dma.get()->ioaddrs[i]) + j * ps;
          // std::cout << std::dec << "\ti: " << i << "\tj: " << j << "\tindex: "<<
          // (i*how_many_in_one + j) << "\t" << std::hex <<
          // (((uint64_t)this->pages_dma.get()->ioaddrs[i]) + j*ps) << std::dec <<
          // std::endl;
        }
      }
      cuda_err_chk(
          cudaMemcpy(pdt.prp1, temp, np * sizeof(uint64_t), cudaMemcpyHostToDevice));
      delete temp;
      // std::cout << "HERE1\n";
      // free(temp);
      // std::cout << "HERE2\n";
    ...
    else if ((ps > this->pages_dma.get()->page_size) &&
             (ps <= (this->pages_dma.get()->page_size * 2))) {
      this->prp1_buf = createBuffer(np * sizeof(uint64_t), cudaDevice);
      pdt.prp1 = (uint64_t *)this->prp1_buf.get();
      ...
      cuda_err_chk(
          cudaMemcpy(pdt.prp1, temp1, np * sizeof(uint64_t), cudaMemcpyHostToDevice));
      cuda_err_chk(
          cudaMemcpy(pdt.prp2, temp2, np * sizeof(uint64_t), cudaMemcpyHostToDevice));
      ...
      this->prp_list_dma =
          createDma(ctrl.ctrl, NVM_PAGE_ALIGN(prp_list_size, 1UL << 16), cudaDevice);
      this->prp2_buf = createBuffer(np * sizeof(uint64_t), cudaDevice);
      pdt.prp2 = (uint64_t *)this->prp2_buf.get();
      uint64_t *temp1 = new uint64_t[np * sizeof(uint64_t)];
      uint64_t *temp2 = new uint64_t[np * sizeof(uint64_t)];
      ...
      for (size_t i = 0; i < np; i++) {
        ...
        for (size_t j = 0; j < (how_many_in_one - 1); j++) {
          temp3[i * uints_per_page + j] =
              ((uint64_t)this->pages_dma.get()->ioaddrs[i * how_many_in_one + j + 1]);
        }
      }
      /*
        ...
        temp3[(i/how_many_in_one)*uints_per_page + j] =
        ((uint64_t)this->pages_dma.get()->ioaddrs[i+1+j]);
        }
        }
      */
      ...
      cuda_err_chk(
          cudaMemcpy(pdt.prp2, temp2, np * sizeof(uint64_t), cudaMemcpyHostToDevice));
      cuda_err_chk(cudaMemcpy(this->prp_list_dma.get()->vaddr, temp3, prp_list_size,
                              cudaMemcpyHostToDevice));
...
struct range_d_t {
  ...
  __forceinline__ __device__ uint64_t get_backing_page(const size_t i) const;
  __forceinline__ __device__ uint64_t get_backing_ctrl(const size_t i) const;
  __forceinline__ __device__ uint64_t get_sector_size() const;
  __forceinline__ __device__ uint64_t get_page(const size_t i) const;
  __forceinline__ __device__ uint64_t get_subindex(const size_t i) const;
  __forceinline__ __device__ uint64_t get_global_address(const size_t page) const;
  __forceinline__ __device__ void release_page(const size_t pg) const;
  __forceinline__ __device__ void release_page(const size_t pg,
                                               const uint32_t count) const;
  __forceinline__ __device__ uint64_t acquire_page(const size_t pg, const uint32_t count,
                                                   const bool write, const uint32_t ctrl,
                                                   const uint32_t queue);
  __forceinline__ __device__ void write_done(const size_t pg, const uint32_t count) const;
  __forceinline__ __device__ T operator[](const size_t i);
  __forceinline__ __device__ void operator()(const size_t i, const T val);
  __forceinline__ __device__ cache_page_t *get_cache_page(const size_t pg) const;
...
struct range_t {
  ...
  range_t(uint64_t is, uint64_t count, uint64_t ps, uint64_t pc, uint64_t pso,
          uint64_t p_size, page_cache_t *c_h, uint32_t cudaDevice,
          data_dist_t dist = REPLICATE);
};

template <typename T>
range_t<T>::range_t(uint64_t is, uint64_t count, uint64_t ps, uint64_t pc, uint64_t pso,
                    uint64_t p_size, page_cache_t *c_h, uint32_t cudaDevice,
                    data_dist_t dist) {
  rdt.access_cnt = 0;
  rdt.miss_cnt = 0;
  ...
  // cudaPointerGetAttributes(&attr, rdt.pages);
  // printf("page count=%d, rdt.pages = %p, attr=%d \n", pc, rdt.pages, attr.type);
  ...
  cuda_err_chk(
      cudaMemcpy(d_range_ptr, &rdt, sizeof(range_d_t<T>), cudaMemcpyHostToDevice));
...
template <typename T>
__forceinline__ __device__ uint64_t range_d_t<T>::get_sector_size() const {
  return page_size;
}

template <typename T>
__forceinline__ __device__ uint64_t range_d_t<T>::get_page(const size_t i) const {
  uint64_t index =
      ((i - index_start) * sizeof(T) + page_start_offset) >> (cache.page_size_log);
  return index;
}
template <typename T>
__forceinline__ __device__ uint64_t range_d_t<T>::get_subindex(const size_t i) const {
  uint64_t index =
      ((i - index_start) * sizeof(T) + page_start_offset) & (cache.page_size_minus_1);
  return index;
}
template <typename T>
__forceinline__ __device__ uint64_t
...
template <typename T>
__forceinline__ __device__ void range_d_t<T>::release_page(const size_t pg) const {
  uint64_t index = pg;
  pages[index].state.fetch_sub(1, simt::memory_order_release);
}

template <typename T>
__forceinline__ __device__ void range_d_t<T>::release_page(const size_t pg,
                                                           const uint32_t count) const {
  uint64_t index = pg;
...
template <typename T>
__forceinline__ __device__ cache_page_t *range_d_t<T>::get_cache_page(
    const size_t pg) const {
...
template <typename T>
__forceinline__ __device__ uint64_t range_d_t<T>::acquire_page(const size_t pg,
                                                               const uint32_t count,
                                                               const bool write,
                                                               const uint32_t ctrl_,
                                                               const uint32_t queue) {
  //   assert(0);
  uint64_t index = pg;
  ...
  do {
    ...
    // NOTE(xieminhui): 取出state最高的两位，分别是VALID和BUSY
    ...
    switch (st) {
      ...
      case NV_NB:
        ...
        if ((st_new & BUSY) == 0) {
          ...
          // uint32_t ctrl = cache.ctrl_counter->fetch_add(1, simt::memory_order_relaxed)
          // % (cache.n_ctrls);
          uint64_t ctrl = get_backing_ctrl(index);
          if (ctrl == ALL_CTRLS)
            ctrl = cache.ctrl_counter->fetch_add(1, simt::memory_order_relaxed) %
                   (cache.n_ctrls);
          // ctrl = ctrl_;
          ...
          // uint32_t queue = c->queue_counter.fetch_add(1, simt::memory_order_relaxed) %
          // (c->n_qps); uint32_t queue = ((sm_id * 64) + warp_id()) % (c->n_qps);
          read_io_cnt.fetch_add(1, simt::memory_order_relaxed);
          read_data(&cache, (c->d_qps) + queue, ((b_page)*cache.n_blocks_per_page),
          ...
          // while (cache.page_translation[global_page].load(simt::memory_order_acquire)
          // != page_trans)
          //     __nanosleep(100);
          // miss_cnt.fetch_add(count, simt::memory_order_relaxed);
          miss_cnt.fetch_add(count, simt::memory_order_relaxed);
          new_state = VALID;
          if (write) pages[index].state.fetch_or(DIRTY, simt::memory_order_relaxed);
          // new_state |= DIRTY;
    ...
    if (fail) {
      ...
      //                (unsigned long long)(CNT_SHIFT + 1),
      //            (unsigned long long)index, (unsigned long long)read_state,
      //            (unsigned long long)st, (unsigned long long)(st_new >> (CNT_SHIFT + 1)) & 0x03);
...
struct array_d_t {
  ...
  __forceinline__ __device__ void get_page_gid(const uint64_t i, range_d_t<T> *&r_,
                                               size_t &pg, size_t &gid) const {
  ...
  __forceinline__ __device__ void memcpy(const uint64_t i, const uint64_t count,
                                         T *dest) {
    uint32_t lane = lane_id();
    int64_t r = find_range(i);
    auto r_ = d_ranges + r;
    ...
    if (r != -1) {
      ...
      if (master == lane) {
        ...
        //                long) threadIdx.x, base_master, (unsigned long long) page);
      }
      base_master = __shfl_sync(eq_mask, base_master, master);

      // if (threadIdx.x == 63) {
      ////printf("--tid: %llu\tpage: %llu\tsubindex: %llu\tbase_master: %llu\teq_mask:
      ///%x\tmaster: %llu\n", (unsigned long long) threadIdx.x, (unsigned long long) page,
      ///(unsigned long long) subindex, (unsigned long long) base_master, (unsigned)
      /// eq_mask, (unsigned long long) master);
      //}
      //
      ...
      __syncwarp(eq_mask);
      if (master == lane) r_->release_page(page, count);
      __syncwarp(mask);
    }
  }
  __forceinline__ __device__ int64_t find_range(const size_t i) const {
    int64_t range = -1;
    int64_t k = 0;
    for (; k < n_ranges; k++) {
      // TODO(xieminhui): maybe BUG
  ...
  __forceinline__ __device__ void coalesce_page(const uint32_t lane, const uint32_t mask,
                                                const int64_t r, const uint64_t page,
                                                const uint64_t gaddr, const bool write,
    ...
    if (lane == leader) {
      page_cache_d_t *pc = &(r_->cache);
      ctrl = 0;  // pc->ctrl_counter->fetch_add(1, simt::memory_order_relaxed) %
                 // (pc->n_ctrls);
  ...
  __forceinline__ __device__ returned_cache_page_t<T> get_raw(const size_t i) const {
    ...
    if (r != -1) {
      ...
      coalesce_page(lane, mask, r, page, gaddr, false, eq_mask, master, count,
                    base_master);
  ...
  __forceinline__ __device__ void release_raw(const size_t i) const {
    uint32_t lane = lane_id();
    int64_t r = find_range(i);
    auto r_ = d_ranges + r;
  ...
  __forceinline__ __device__ void *acquire_page_(const size_t i, data_page_t *&page_,
    ...
    if (r_) {
      ...
      uint64_t gaddr = r_->get_global_address(page);
      page_cache_d_t *pc = &(r_->cache);
      uint32_t ctrl = 0;  // pc->ctrl_counter->fetch_add(1, simt::memory_order_relaxed) %
                          // (pc->n_ctrls);
  ...
  __forceinline__ __device__ void *acquire_page(const size_t i, data_page_t *&page_,
                                                size_t &start, size_t &end,
                                                int64_t &r) const {
    uint32_t lane = lane_id();
    r = find_range(i);
  ...
  __forceinline__ __device__ void release_page(data_page_t *page_, const int64_t r,
  ...
  __forceinline__ __device__ T seq_read(const size_t i) const {
    uint32_t lane = lane_id();
    int64_t r = find_range(i);
    auto r_ = d_ranges + r;
    T ret;
    ...
    if (r != -1) {
      ...
      printf("[seq_read, find_range]out of range, i=%lld\n ", i);
      *(int *)0 = 0;
    }
    return ret;
  }
  __forceinline__ __device__ void seq_write(const size_t i, const T val) const {
    uint32_t lane = lane_id();
    int64_t r = find_range(i);
    auto r_ = d_ranges + r;
    ...
    if (r != -1) {
      ...
      coalesce_page(lane, mask, r, page, gaddr, true, eq_mask, master, count,
                    base_master);
  ...
  __forceinline__ __device__ T operator[](size_t i) const {
    return seq_read(i);
    // size_t k = 0;
    // bool found = false;
    // for (; k < n_ranges; k++) {
  ...
  __forceinline__ __device__ void operator()(size_t i, T val) const {
    seq_write(i, val);
    // size_t k = 0;
    // bool found = false;
    // uint32_t mask = __activemask();
  ...
  __forceinline__ __device__ T AtomicAdd(const size_t i, const T val) const {
    ...
    if (r != -1) {
      ...
      if (master == lane) {
        ...
        //    long) threadIdx.x, (unsigned long long) base_master, (unsigned long long)
        //    memcpyflag_master);
      }
      base_master = __shfl_sync(eq_mask, base_master, master);
      ...
      // //printf("AtomicAdd: tid: %llu\tpage: %llu\tsubindex: %llu\tval: %llu\told_val:
      // %llu\tbase_master: %llx\n",
      //        (unsigned long long) tid, (unsigned long long) page, (unsigned long long)
      //        subindex, (unsigned long long) val,
...
struct array_t {
  ...
  void print_reset_stats(void) {
    ...
    cuda_err_chk(cudaMemcpy(adt.d_ranges, rdt.data(), adt.n_ranges * sizeof(range_d_t<T>),
                            cudaMemcpyHostToDevice));
  }

  array_t(const uint64_t num_elems, const uint64_t disk_start_offset,
    ...
    for (size_t k = 0; k < adt.n_ranges; k++) {
      // cuda_err_chk(cudaMemcpy(d_ranges+k, &(ranges[k]->d_range_ptr),
      // sizeof(range_t<T>*), cudaMemcpyHostToDevice));
      cuda_err_chk(cudaMemcpy(adt.d_ranges + k, (ranges[k]->d_range_ptr),
                              sizeof(range_d_t<T>), cudaMemcpyDeviceToDevice));
    }

    cuda_err_chk(
        cudaMemcpy(d_array_ptr, &adt, sizeof(array_d_t<T>), cudaMemcpyHostToDevice));
  }
};

__forceinline__ __device__ cache_page_t *page_cache_d_t::get_cache_page(
    const uint32_t page) const {
  return &this->cache_pages[page];
}

__forceinline__ __device__ uint32_t page_cache_d_t::find_slot(uint64_t address,
                                                              uint64_t range_id,
                                                              const uint32_t queue_) {
  bool fail = true;
  uint64_t count = 0;
  uint32_t global_address =
      (uint32_t)((address << n_ranges_bits) | range_id);  // not elegant. but hack
  uint32_t page = 0;
  unsigned int ns = 8;
  uint64_t j = 0;
  uint64_t expected_state = VALID;
  ...
  do {
    ...
    //     page = (page/n_cachelines_for_states) +
    //     ((page%n_cachelines_for_states)*STATES_PER_CACHELINE);
    // }
    ...
    // this->page_take_lock[page].compare_exchange_strong(unlocked, LOCKED,
    // simt::memory_order_acquire, simt::memory_order_relaxed); not assigned to anyone yet
    if (v == FREE) {
      lock = this->cache_pages[page].page_take_lock.compare_exchange_weak(
      ...
      if (lock) {
        ...
        this->cache_pages[page].page_take_lock.store(UNLOCKED,
                                                     simt::memory_order_release);
        fail = false;
      }
    }
    // assigned to someone and was able to take lock
      ...
      if (lock) {
        ...
        //     //printf("prev_ga: %llu\tprev_range: %llu\tprev_add: %llu\trange_cap:
        //     %llu\tn_pages: %llu\n", (unsigned long long) previous_global_address,
        //     (unsigned long long) previous_range, (unsigned long long) previous_address,
        //            (unsigned long long) range_cap, (unsigned long long) n_pages);
        expected_state = this->ranges[previous_range][previous_address].state.load(
            simt::memory_order_relaxed);
        ...
        if ((cnt == 0) && (b == 0)) {
          new_expected_state =
              this->ranges[previous_range][previous_address].state.fetch_or(
                  BUSY, simt::memory_order_acquire);
          if (((new_expected_state & BUSY) == 0)) {
            // while ((new_expected_state & CNT_MASK ) != 0) new_expected_state =
            // this->ranges[previous_range][previous_address].state.load(simt::memory_order_acquire);
            if (((new_expected_state & CNT_MASK) == 0)) {
              if ((new_expected_state & DIRTY)) {
                uint64_t ctrl = get_backing_ctrl_(previous_address, n_ctrls,
                                                  ranges_dists[previous_range]);
                // uint64_t get_backing_page(const uint64_t page_start, const size_t
                // page_offset, const uint64_t n_ctrls, const data_dist_t dist) {
                uint64_t index = get_backing_page_(ranges_page_starts[previous_range],
                                                   previous_address, n_ctrls,
                                                   ranges_dists[previous_range]);
                // //printf("Eviciting range_id: %llu\tpage_id: %llu\tctrl: %llx\tindex:
                // %llu\n",
                //        (unsigned long long) previous_range, (unsigned long
                //        long)previous_address, (unsigned long long) ctrl, (unsigned long
                //        long) index);
                if (ctrl == ALL_CTRLS) {
                  for (ctrl = 0; ctrl < n_ctrls; ctrl++) {
                    Controller *c = this->d_ctrls[ctrl];
                    uint32_t queue = queue_ % (c->n_qps);
                    write_data(this, (c->d_qps) + queue,
                               (index * this->n_blocks_per_page), this->n_blocks_per_page,
                               page);
                  }
                } else {
                  Controller *c = this->d_ctrls[ctrl];
                  ...
                  write_data(this, (c->d_qps) + queue, (index * this->n_blocks_per_page),
                             this->n_blocks_per_page, page);
                }
              }

              fail = false;
              this->ranges[previous_range][previous_address].state.fetch_and(
                  CNT_MASK, simt::memory_order_release);
            } else {
              this->ranges[previous_range][previous_address].state.fetch_and(
                  DISABLE_BUSY_MASK, simt::memory_order_release);
              // if ((j % 1000000) == 0) {
              //                 printf("failed to find slot j: %llu\taddr: %llx\tpage:
              //                 %llx\texpected_state: %llx\tnew_expected_date: %llx\n",
              //                 (unsigned long long) j, (unsigned long long) address,
              //                 (unsigned long long)page, (unsigned long long)
              //                 expected_state, (unsigned long long) new_expected_state);
              // }
            }
          }
        }

        // this->ranges[previous_range][previous_address].compare_exchange_strong(expected_state,
        // new_state, simt::memory_order_acquire, simt::memory_order_relaxed);
...
#endif
      //   if ((j % 10000000) == 0) {
      //     printf("failed to find slot j: %llu\taddr: %llx\tpage: %llx\texpected_state:
      //     %llx\tnew_expected_date: %llx\n", (unsigned long long) j, (unsigned long
      //     long) address, (unsigned long long)page, (unsigned long long) expected_state,
      //     (unsigned long long) new_expected_state);
      //            }
      //	   expected_state = 0;
...
inline __device__ void access_data_async(page_cache_d_t *pc, QueuePair *qp,
                                         const uint64_t starting_lba,
                                         const uint64_t n_blocks,
                                         const unsigned long long pc_entry,
                                         const uint8_t opcode, uint16_t *cid,
                                         uint16_t *sq_pos) {
  nvm_cmd_t cmd;
  *cid = get_cid(&(qp->sq));
  ////printf("cid: %u\n", (unsigned int) cid);
  ...
  /// long) (threadIdx.x+blockIdx.x*blockDim.x), (unsigned long long) starting_lba,
  ///(unsigned long long) n_blocks, (void*) prp1);
  nvm_cmd_data_ptr(&cmd, prp1, prp2);
  nvm_cmd_rw_blks(&cmd, starting_lba, n_blocks);
  *sq_pos = sq_enqueue(&qp->sq, &cmd);
}

inline __device__ void enqueue_second(page_cache_d_t *pc, QueuePair *qp,
                                      const uint64_t starting_lba, nvm_cmd_t *cmd,
                                      const uint16_t cid, const uint64_t pc_pos,
...
#if defined(__CUDACC__) && (__CUDA_ARCH__ >= 700 || !defined(__CUDA_ARCH__))
    __nanosleep(ns);
    // printf("stall in enqueue_second\n");
    if (ns < 256) {
      ns *= 2;
    }
#endif
...
inline __device__ void read_data(page_cache_d_t *pc, QueuePair *qp,
  ...
  // enqueue_second(page_cache_d_t* pc, QueuePair* qp, const uint64_t starting_lba,
  // nvm_cmd_t* cmd, const uint16_t cid, const uint64_t pc_pos, const uint64_t
  // pc_prev_head)
...
inline __device__ void write_data(page_cache_d_t *pc, QueuePair *qp,
                                  const uint64_t starting_lba, const uint64_t n_blocks,
                                  const unsigned long long pc_entry) {
  // uint64_t starting_lba = starting_byte >> qp->block_size_log;
  // uint64_t rem_bytes = starting_byte & qp->block_size_minus_1;
...
inline __device__ void access_data(page_cache_d_t *pc, QueuePair *qp,
                                   const uint64_t starting_lba, const uint64_t n_blocks,
                                   const unsigned long long pc_entry,
