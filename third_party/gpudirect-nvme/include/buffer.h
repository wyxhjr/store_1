...
DmaPtr createDma(const nvm_ctrl_t *ctrl, size_t size, int cudaDevice, uint32_t adapter,
                 uint32_t id);
...
static void getDeviceMemory(int device, void *&bufferPtr, void *&devicePtr, size_t size,
  ...
  */
  ...
  devicePtr =
      (void *)((((uint64_t)attrs.devicePointer) + (64 * 1024)) & 0xffffffffff0000);
...
static void getDeviceMemory2(int device, void *&bufferPtr, size_t size, void *&origPtr) {
  ...
  /*
      ...
      {
          cudaFree(bufferPtr);
          throw error(string("Failed to get pointer attributes: ") +
...
inline BufferPtr createBuffer(size_t size, int cudaDevice) {
  if (cudaDevice < 0) {
    std::cout << "WARNINIG createBuffer with cudaDeivce < 0";
