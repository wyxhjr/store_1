...
int _nvm_dma_init(nvm_dma_t** handle, const nvm_ctrl_t* ctrl, struct va_range* va, va_range_free_t release)
{
    *handle = NULL;
    ...
    if (release == NULL)
    {
        printf("[_nvm_dma_init] release==NULL failed\n");
        return EINVAL;
    }
    ...
    if (err != 0)
    {
        printf("[_nvm_dma_init] create_map failed\n");
        return err;
    }
    ...
    if (err != 0)
    {
        printf("[_nvm_dma_init] create_container failed \n");
        remove_map(map);
        return err;
    }

    // Map controller for device and populate handle
    // printf("[_nvm_dma_init] dma_map\n");
    err = dma_map(container);
    if (err != 0)
    {
        printf("[_nvm_dma_init] dma_map failed, err=%d\n", err);
        remove_map(map);
        free(container);
        return err;
    }
    // printf("[_nvm_dma_init] dma_map done\n");
